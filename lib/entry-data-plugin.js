...
/**
 * Normalize a deployment base path so it always starts and ends with `/`.
 * Examples: "" / "/" -> "/", "squoosh" -> "/squoosh/",
 * "/squoosh" -> "/squoosh/", "/squoosh/" -> "/squoosh/".
 *
 * The app can be deployed to a subdirectory of a website (e.g. a GitHub
 * Pages project site at /<repo>/). All built asset URLs are prefixed with
 * this base so they resolve correctly under the subdirectory. Defaults to
 * root deployment (`/`).
 *
 * @param {string} base
 * @returns {string}
...
export function fileNameToURL(fileName) {
  return basePath + fileName.replace(/^static\//, '');
}
