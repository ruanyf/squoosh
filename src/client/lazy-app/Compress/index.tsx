      ...
      try {
        ...
        this.setState((currentState) => {
          ...
          const sides = currentState.sides.map((side) => {
            // Auto-open the resize menu and set width to 800 so compression
            // is triggered automatically on image load.
            const autoResizeWidth = 800;
            const autoResizeHeight = Math.round(
              (autoResizeWidth * decoded.height) / decoded.width,
            );
            const resizeState: Partial<ProcessorState['resize']> = {
              width: autoResizeWidth,
